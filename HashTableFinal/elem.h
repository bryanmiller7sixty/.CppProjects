...
class el_t
{
 ...
 public:
  ...
  el_t(int, string, double, string); // to create an initialized el_t object - useful for the client
 ...
 // use different el_t without modifying the code
  ...
  bool operator!=(el_t);  // overload != for search
  void operator=(const el_t&);  //overload to assign data to a new object
